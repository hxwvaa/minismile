...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/19 13:59:50 by hbasheer          #+#    #+#             */
/*   Updated: 2024/12/19 13:59:50 by hbasheer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//---------------------pre_execute------------------//
t_toklist				*array_token_list(t_shell *data, char **split,
							int count);
...
char					*handle_expand(char *line, t_shell *data, int *len,
							char *res);

//---------------------execution------------------//
void					pre_execute(t_shell *data, int input, int output);
void					hd_clean_exit(int exit_code, int flag, int pipefd[],
							t_shell *data);
...
void					set_redirection(t_cmd *curr, t_shell *data, int *input,
							int *output);
void					fork_execute_child(t_shell *data, t_cmd *curr,
							int *input, int *output);
void					builtin_pipeline(t_cmd *curr, t_shell *data);
void					prepare_fds(int *input, int *output, t_shell *data,
							t_cmd *curr);
void					close_clean(t_shell *data, int input, int output);
void					invalid_lstcmd(char *file, int *input, int *output,
							t_shell *data);
void					invalid_cmd_dir(char *file, int *input, int *output,
							t_shell *data);
int						check_if_directory(char *cmd);
void					wait_loop(t_shell *data, int status, pid_t pid);
char					*get_cmd_path(char *cmd, int *input, int *output,
							t_shell *data);
...
void					free_exec_fail(t_shell *data, int *input, int *output,
							int exit_code);
